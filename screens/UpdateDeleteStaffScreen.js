...
export default function UpdateDeleteStaffScreen({ route, navigation }) {
  const { staff, username, role, isOnline = true } = route.params || {};
  ...
  const [staffShifts, setStaffShifts] = useState(staff.shift || []);

  // Role dropdown
  const [staffRole, setStaffRole] = useState(staff.role || "");
  ...
  const handleUpdate = async () => {
    ...
    // Required fields
    if (!firstName || !lastName || !email || !phone || !staffRole) {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/user/update/${staff.id}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        setErrorMessage(json.error || "Failed to update staff");
        return;
      }
          ...
          onPress: async () => {
            try {
              ...
              const response = await fetch(
                `${API_BASE}/api/user/delete/${staff.id}`,
                {
                  method: "DELETE",
                  headers: {
                    Authorization: `Bearer ${token}`,
              ...
              if (!response.ok) {
                setErrorMessage(json.error || "Failed to delete staff");
                return;
              }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.mainContent} contentContainerStyle={{ paddingBottom: 40 }}>
        <Text style={styles.screenTitle}>Update/Delete Staff</Text>
        ...
        <View style={isTablet ? styles.tabletCenterWrapper : null}>
          <View style={isTablet ? styles.tabletContent : null}>
            ...
            <View style={[styles.displayBox, isTablet && styles.inputFieldTablet]}>
              <Text style={styles.displayText}>{staff.username}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  updateBtnText: { color: COLORS.white, fontSize: 18, fontWeight: "bold" },
  ...
  deleteBtnText: { color: COLORS.white, fontSize: 18, fontWeight: "bold" },
