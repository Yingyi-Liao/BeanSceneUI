...
export default function UpdateStaffScreen({ isOnline = true, onBack }) {
  ...
  // Mock existing staff data
  const [staffName, setStaffName] = useState('Alice Johnson');
  const [staffEmail, setStaffEmail] = useState('alice@example.com');
  const [staffPhone, setStaffPhone] = useState('0400 123 456');

  // Multi-shift
  const [selectedShift, setSelectedShift] = useState('');
  const [staffShifts, setStaffShifts] = useState(['Morning', 'Evening']);
...
const styles = StyleSheet.create({
  ...
  updateBtnText: { color: 'white', fontSize: 18, fontWeight: 'bold' },
